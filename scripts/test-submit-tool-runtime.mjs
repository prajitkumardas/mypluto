...
await waitFor("document.querySelector('[role=dialog] h2')?.textContent.includes('Submit an AI tool')");
await delay(400);
const desktop = await evaluate("(() => { const rect=document.querySelector('[role=dialog]').getBoundingClientRect(); const cat=document.querySelector('[role=dialog] img[src*=\"plutopopup\"]'); const catRect=cat?.getBoundingClientRect(); return { width:rect.width, left:rect.left, right:innerWidth-rect.right, top:rect.top, bottom:innerHeight-rect.bottom, title:document.querySelector('[role=dialog] h2')?.textContent, catWidth:catRect?.width, catTop:catRect?.top, catBottom:catRect?.bottom, catLoaded:cat?.complete && cat?.naturalWidth > 0 }; })()");
assert.ok(desktop.width <= 800 && Math.abs(desktop.left - desktop.right) < 2, "Desktop modal is centered and capped at 800px");
assert.match(desktop.title, /Submit an AI tool/);
assert.ok(desktop.catLoaded && desktop.catWidth >= 120, "Desktop cat artwork is loaded at the Play popup scale");
assert.ok(desktop.catTop < desktop.top && desktop.catBottom > desktop.top, "Desktop cat overlaps the modal's top edge");
...
assert.equal(await clickButton("Submit a Tool"), true);
await waitFor("Boolean(document.querySelector('[role=dialog]'))");
await waitFor("[...document.querySelectorAll('[role=dialog] button')].some((item) => item.textContent.includes(\"Let's submit\"))");
assert.equal(await clickButton("Let's submit"), true);
...
const firstCategory = await evaluate("document.querySelector('select[name=category] option:not([value=\"\"])')?.value");
const firstPricing = await evaluate("document.querySelector('select[name=pricing] option:not([value=\"\"])')?.value");
assert.ok(firstCategory && firstPricing);
await setControl("select[name=category]", firstCategory);
await setControl("select[name=pricing]", firstPricing);
...
await setControl("select[name=relationship]", "User");
await clickButton("Continue");
...
assert.equal(await clickButton("Submit a Tool"), true);
await waitFor("Boolean(document.querySelector('[role=dialog]'))");
await waitFor("(() => { const image=document.querySelector('[role=dialog] img[src*=\"plutopopup\"]'); return image?.complete && image?.naturalWidth > 0; })()");
await delay(400);
const mobile = await evaluate("(() => { const rect=document.querySelector('[role=dialog]').getBoundingClientRect(); const cat=document.querySelector('[role=dialog] img[src*=\"plutopopup\"]'); const catRect=cat?.getBoundingClientRect(); return { width:rect.width, left:rect.left, right:innerWidth-rect.right, scrollWidth:document.documentElement.scrollWidth, viewport:innerWidth, maxHeight:getComputedStyle(document.querySelector('[role=dialog]')).maxHeight, catWidth:catRect?.width, catTop:catRect?.top, catBottom:catRect?.bottom, modalTop:rect.top, catLoaded:cat?.complete && cat?.naturalWidth > 0 }; })()");
...
assert.ok(mobile.catLoaded && mobile.catWidth >= 82, "Mobile cat artwork remains visible at a responsive scale");
assert.ok(mobile.catTop >= 0 && mobile.catBottom > mobile.modalTop, "Mobile cat stays within the viewport and overlaps the modal");
