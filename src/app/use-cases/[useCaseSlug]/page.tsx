...
export default async function UseCasePage({ params }: UseCasePageProps) {
  ...
  return (
    <main className="mx-auto max-w-site px-5 py-14 sm:px-8 lg:py-20 xl:px-0">
      ...
      <p className="mt-4 max-w-2xl text-lg leading-8 text-neutral-700">{useCase.query}</p>
      <div className="mt-8 grid gap-4 md:grid-cols-4">
        {useCase.groups.map((group) => (
          <div className="rounded-2xl bg-lime-100 p-4 text-sm font-bold text-ink-950" key={group}>
            {group}
          </div>
        ))}
      </div>
