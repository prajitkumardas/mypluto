...
void main() {
  ...
  float mouseWarp = 0.0;
  if (uEnableMouse) {
    vec2 mPos = uMouse * 2.0 - 1.0;
    mPos.x *= uResolution.z;
    mPos = rotate2D(mPos, uRotation);
  ...
  vec2 blendFactor = clamp(mix(fieldA, fieldB, 0.5) * 0.5 + 0.5, 0.0, 1.0);
  ...
  float alpha = clamp(length(col), 0.0, 0.92);
...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, premultipliedAlpha: false, dpr: Math.min(window.devicePixelRatio, 1.75) });
    ...
    function resize() {
      ...
      program.uniforms.uResolution.value = [gl.canvas.width, gl.canvas.height, width / height];
    }
